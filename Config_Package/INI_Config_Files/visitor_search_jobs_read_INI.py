...
class Read_Visitor_Search_jobs_Components:
    def __init__(self):
        ...
        try:
            vsj_ini_file_path = f'{Path(__file__).parent.parent.parent}\\Test_Data\\Data_From_INI' \
                                f'\\Visitor_search_jobs.ini'
    ...
    def vsj_panel_title(self):
        try:
            ele = self.config.get("VALIDATION", "vsj_panel_title")
            return ele
        except Exception as ex:
    ...
    def connection_error(self):
        try:
            ele = self.config.get("DATA", "connection_error")
            return ele
        except Exception as ex:
